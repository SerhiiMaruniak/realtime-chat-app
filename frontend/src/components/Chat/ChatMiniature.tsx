...
import SidebarContextMenu from "./Message/SidebarContextMenu.tsx";
...
const ChatMiniature = ({ user, width, min_width, messages }: ChatMiniatureProps) => {
  ...
  const { selectedChat, selectChat, sidebarContextMenu, showSidebarContextMenu } =
    useChatStore();
  ...
  return (
    <>
      ...
      {sidebarContextMenu?.user === user._id && <SidebarContextMenu user={user} />}
    </>
  );
};
